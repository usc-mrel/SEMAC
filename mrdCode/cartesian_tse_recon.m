function [im,header,x,y,z,imc,csm] = cartesian_tse_recon(noise_fullpath, cartesian_fullpath, dat_fullpath, flag)
% Inputs
...
if flag.TF >1
    flag.ignoreSeg = 'true';
    ESP         = header.sequenceParameters.echo_spacing * 1e-3;  % [msec] * [sec/1e3msec] => [sec]
    idx_effectiveTE = TE*1e3/round(ESP*1e3);
end
...
if Nky ~= nr_phase_encodings
    Nky = nr_phase_encodings;
end
if Ny ~= nr_phase_encodings
    Ny = nr_phase_encodings;
end
%--------------------------------------------------------------------------
...
for idx = 1:nr_recons
    ...
    if strcmp(flag.ignoreSeg,'true')
        temp = squeeze(data(1, :, :, average_nr, slice_nr, contrast_nr, phase_nr, repetition_nr, set_nr, idx_effectiveTE, 1));
        LFPhaseEncoding_region = find(abs(temp)~=0);
        temp2 = sum(data(:, :, :, average_nr, slice_nr, contrast_nr, phase_nr, repetition_nr, set_nr, :, :), 10);
        temp2(:,LFPhaseEncoding_region(1):LFPhaseEncoding_region(end),:,:,:,:,:,:,:,:,:) = ...
            data(:, LFPhaseEncoding_region(1):LFPhaseEncoding_region(end), :, average_nr, slice_nr, contrast_nr, phase_nr, repetition_nr, set_nr, idx_effectiveTE, :);
        data = temp2;
        clear temp2
    end
    ...
    kspace = ipermute(reshape(inv_L * reshape(permute(kspace,[4 1 2 3]), [Nc Nkx*nr_phase_encodings*Nkz]), [Nc Nkx nr_phase_encodings Nkz]), [4 1 2 3]);
    ...
    % Siemens use forward FFT to move from k-space to image-space
    %----------------------------------------------------------------------
    imc_ = fftshift(fft(ifftshift(imc_, 2), [], 2), 2); % Nx x Ny x Nkz x Nc
    
    %----------------------------------------------------------------------
    % Save coil images
    %----------------------------------------------------------------------
    imc(:, :, :, average_nr, slice_nr, contrast_nr, phase_nr, repetition_nr, set_nr, segment_nr, :) = imc_;
    fprintf('done! (%6.4f/%6.4f sec)\n', toc, toc(start_time));
    
    %% Calculate coil sensitivity maps
    tic; fprintf('(%2d/%2d): Calculating coil sensitivity maps with %1s method... ', idx, nr_recons,flag.csm_method );
    ...
    %----------------------------------------------------------------------
    cal_shape = flag.cal_shape;
    cal_data = crop(reshape(kspace, [Nx Ny Nc]), [cal_shape Nc]);
    
    if strcmp(flag.csm_method, 'sos')
        % hamming_window = zeros(N1, N2, 'double');
        % idx1_range = (-floor(cal_shape(1)/2):ceil(cal_shape(1)/2)-1).' + floor(N1/2) + 1;
        % idx2_range = (-floor(cal_shape(2)/2):ceil(cal_shape(2)/2)-1).' + floor(N2/2) + 1;
        % hamming_window(idx1_range,idx2_range) = hamming(cal_shape(1)) * hamming(cal_shape(2)).';
        % kspace = bsxfun(@times, kspace, hamming_window);
        cal_im = fft2c(zpad(cal_data, [Nx Ny Nc]));
        csm_ = estimate_SoS(cal_im);
        csm_ = reshape(csm_, [Nx Ny Nz Nc]);
        fprintf('done! (%6.4f/%6.4f sec)\n', toc, toc(start_time));
    elseif strcmp(flag.csm_method, 'walsh')
    ...
    %% Save CSM
    csm(:, :, :, average_nr, slice_nr, contrast_nr, phase_nr, repetition_nr, set_nr, segment_nr, :) = csm_;
    
    %% Perform optimal coil combination
    im(:, :, :, average_nr, slice_nr, contrast_nr, phase_nr, repetition_nr, set_nr, segment_nr) = sum(conj(csm_) .* imc_, 4) ./ sum(abs(csm_).^2, 4);
end
